...
{
    { "Forward, gripper level",     200, 0, 200, 45 },
...
//--------------------------------------------------
// computeIK() produces angles in an abstract math frame where
// 0 degrees means: shoulder horizontal, elbow fully extended
// (straight), and gripper level -- ALL FOUR joints read exactly
// 0 degrees simultaneously at ONE single physical pose: arm
// fully extended, horizontal, pointing in whatever direction
// you call "straight ahead", gripper level.
//
// Your calibrated raw servo degrees don't share that zero point,
// so we need a simple linear mapping per joint:
//
//     rawDegrees = zeroRawDegrees + sign * mathDegrees
//
// HOW TO FIND THESE VALUES (arm stays fully assembled):
//   1. In normal joystick mode, jog Base/Shoulder/Elbow/Wrist
//      Pitch until the WHOLE ARM is fully extended in a straight
//      line, completely horizontal, gripper level, pointing in
//      the direction you want to call "straight ahead."
//   2. Type 'p' + Enter in Serial Monitor (read-only, does NOT
//      touch calibration/EEPROM). Copy the 4 raw numbers printed
//      into the 4 ZeroRaw constants below.
//   3. For each joint's sign: nudge ONLY that joint a small,
//      known amount, then type 'p' again and compare:
//        Base:     swing it slightly to one side. Pick a side to
//                  call "positive Y" -- doesn't matter which,
//                  it's your own convention. If raw went UP,
//                  sign = 1. If DOWN, sign = -1.
//        Shoulder: lift the arm slightly UP from horizontal.
//                  Raw went UP -> sign = 1. DOWN -> sign = -1.
//        Elbow:    bend the elbow slightly (fold in from full
//                  extension). Raw went UP -> sign = 1. DOWN -> -1.
//        Wrist:    tilt the gripper tip slightly UP.
//                  Raw went UP -> sign = 1. DOWN -> sign = -1.
//--------------------------------------------------

float baseZeroRaw     = 135;   // PLACEHOLDER -- replace via steps above
int   baseSign        = 1;
...
float mapBaseToRaw(float mathDeg)     { return baseZeroRaw     + baseSign     * mathDeg; }
...
void ikTestUpdate()
{
    ...
    if (reachable)
    {
        Serial.print("Math angles -> base=");     Serial.print(baseDeg);
        Serial.print(" shoulder=");                Serial.print(shoulderDeg);
        Serial.print(" elbow=");                   Serial.print(elbowDeg);
        Serial.print(" wrist=");                   Serial.println(wristPitchDeg);

        // Convert math-frame angles to your actual raw servo degrees
        baseDeg     = mapBaseToRaw(baseDeg);
        shoulderDeg = mapShoulderToRaw(shoulderDeg);
        elbowDeg    = mapElbowToRaw(elbowDeg);
        wristPitchDeg = mapWristToRaw(wristPitchDeg);

        Serial.print("Raw angles  -> base=");     Serial.print(baseDeg);
        Serial.print(" shoulder=");                Serial.print(shoulderDeg);
        Serial.print(" elbow=");                   Serial.print(elbowDeg);
        Serial.print(" wrist=");                   Serial.println(wristPitchDeg);
    }
...
//k - enter Step mode, w/s =up/down a/d = Left/right
//+/- step size, 1= toggle autolevel
