...
const float WRIST_ROLL_HOME   = 90.0; //NOW GRIPPER
...
const float WRIST_PITCH_MAX = 245;

const float WRIST_ROLL_MIN = 0;
const float WRIST_ROLL_MAX = 90; //GRIPPER
...
const float WRIST_ROLL_SPEED   = 2; //GRIPPER
...
const byte AXIS_GRIPPER = J3_X; //UNUSED
const byte AXIS_WRIST_ROLL = J2_Y; //GRIPPER
...
const bool INVERT_WRIST_ROLL = false;
...
void ikArmPrintStatus()
{
    ...
    Serial.println("           J2_Y = wrist roll, J3_Y = wrist pitch (manual, auto-level OFF only),");
    Serial.println("           J3_X = gripper, J3 button = toggle auto-level");
...
void ikArmUpdate()
{
    ...
    //--------------------------------------------------
    int wristRollOffset = readAxisOffset(AXIS_WRIST_ROLL, INVERT_WRIST_ROLL);
    joints[WRIST_ROLL].update(centerFor(AXIS_WRIST_ROLL) + wristRollOffset, centerFor(AXIS_WRIST_ROLL));
    joints[WRIST_ROLL].write();

    int gripperOffset = readAxisOffset(AXIS_GRIPPER, INVERT_GRIPPER);
    joints[GRIPPER].update(centerFor(AXIS_GRIPPER) + gripperOffset, centerFor(AXIS_GRIPPER));
    joints[GRIPPER].write();
    ...
    //--------------------------------------------------
    bool j3Pressed = (digitalRead(J3_SW) == LOW);

    if (j3Pressed && !lastJ3ButtonState)
    {
